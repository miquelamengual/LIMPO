...
def get3val(ordereddict):
    ...
    final_cleaner = input("Enter the name of your desired cleaner: ")
...
def main():
    ...
    validwhile1 = False
    while not validwhile1:
        try:
            if options == "YES" or options == "y":
                login()
            validwhile1 = True
        except:
            print("\nSorry something went wrong in the login, please try again")
    validwhile2 = False
    while not validwhile2:
        try:
            if options == "NO" or options == "n":
                regis()
            validwhile2 = True
        except:
            print("\nSorry something went wrong in the registration")

    validwhile3 = False
    while not validwhile3:
        try:
            x1, x2, x3, x4, x5 = book_info()

            cleanerates = cleaner(x1, x2, x3, x4, x5)
            orderedcleaners = quicksort(cleanerates)
            get3val(orderedcleaners)
            validwhile3 = True
        except:
            print("\nSorry something went wrong")
    validwhile4 = False
    while not validwhile4:
        try:
            pay()
            validwhile4 = True
        except:
            print("\nSorry something went wrong in the paying method, try again")
